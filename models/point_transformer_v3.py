...
# This is the implementation of the Point Transformer model v3, extracted from Pointcept and rewrite lots of functions.
"""
...
from .model_utils.serialization import encode
...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        # norm layers
        # NOTE: THIS `conditions` looks very dangerous. It is associated to dataset.
        if pdnorm_bn:
            bn_layer = partial(
                PDNorm,
                norm_layer=partial(
            ...
            bn_layer = partial(
                nn.BatchNorm1d, eps=1e-3, momentum=0.01, track_running_stats=False
            )
        if pdnorm_ln:
            ln_layer = partial(
                PDNorm,
